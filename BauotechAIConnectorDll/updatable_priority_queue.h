...
static std::mutex g_heap_mutex;  // Add for multi threading B.K.
	...
	template <typename Key, typename Priority>
		class updatable_priority_queue {
			...
			public:
				...
				/*--------------------------------------------------------------------------------------
				 * Check if key (camID) is ion top 'resourceLen' of queue
				---------------------------------------------------------------------------------------*/
				int inTop(int key) const {
					int resourceLen = 4;
					int camID = key;
					auto p = std::find_if(heap.begin(), heap.end(),
							[&camID](const priority_queue_node<Key, Priority>& element) { return element.key == camID; });
					bool debug = (p == heap.begin() + resourceLen);
					return (p == heap.begin() + resourceLen) ? false : true;
				}
				...
				Priority get_priority2(const Key& key) {
					if (key < id_to_heappos.size()) {
						size_t pos = id_to_heappos[key];
						if (pos < ((size_t)-2)) {
				...
				bool update(const Key& key, const Priority& new_priority, bool only_if_higher=false) {
					const std::lock_guard<std::mutex> lock(g_heap_mutex);
				...
				// NEW METHODES 
								// Location of key in heap
				int location(int camID)
				{
					return  id_to_heappos[camID];
				}

				std::vector<priority_queue_node<Key, Priority>> status()
				{
					std::sort(heap.begin(), heap.end(), [](const priority_queue_node<Key, Priority>& a, const priority_queue_node<Key, Priority>& b)
						{
				...
				// Increase priority of all keys in the heap by '1'cycles' param
				void beat( int cycles = 1)
				{
					const std::lock_guard<std::mutex> lock(g_heap_mutex);
					for (auto& node : heap)
						node.priority+= cycles;
				}
