...
int** array_to_matrix(std::vector<std::vector<double>>& DistMatrix) 
{
	int i, j;
	int** r;

	unsigned int rows = DistMatrix.size();
	unsigned int cols = DistMatrix[0].size();
...
double Solve2(std::vector<std::vector<double>>& DistMatrix, std::vector<int>& Assignment)
{
	...
	*/
	int** m = array_to_matrix(DistMatrix);
